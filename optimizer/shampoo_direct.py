...
class ShampooHyperParams:
        ...
        statistics_compute_steps: int = 10
...
class Preconditioner:
        ...
        def compute_preconditioners(self):
                ...
                for i, stat in enumerate(self.statistics):
                        ...
                        # print("symmetric=", torch.norm(stat-stat.T))
                        self.preconditioners[i] = matrix_neg_power(stat, exp)
                # it = it + 1
                # print(it)
...
@torch.no_grad()
def matrix_neg_power_evd(mat_g, p):
    S, V = torch.linalg.eigh(mat_g)
    S_neg_power = torch.diag(S ** (-1 / p))
    return V @ S_neg_power @ V.t()
